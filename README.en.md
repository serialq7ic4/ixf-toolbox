...
The v1.x line still uses the Python package as the default installation path and reference runtime. The Go CLI is migrating in stages: it now covers `--version`, `doctor`, `setup skills`, local `docs outline/chunk/inspect/cleanup`, `update check/self`, checksum-verified Go binary self-update, and `update skills`; cookie export, remote docs read/publish, and OKR read/write remain Python-reference behavior until golden parity is complete.

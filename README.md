...
> 请帮我安装 https://github.com/serialq7ic4/ixf-toolbox。使用 GitHub Release Go 二进制安装本地 `ixf`（macOS Apple Silicon 用 `ixf_3.13.0_darwin_arm64`，macOS Intel 用 `ixf_3.13.0_darwin_amd64`，Windows 用 `ixf_3.13.0_windows_amd64.exe`），然后运行 `ixf setup skills --runtimes codex --json` 注册 skill，最后用 `ixf --version` 和 `ixf doctor --json` 验证。
...
macOS Intel 将文件名换成 `ixf_3.13.0_darwin_amd64`。
...
| `ixf sheets update --apply` | 尚不可用；等待真实 sheet 写入 API 契约捕获并补齐 fixture 覆盖 |
...
直接 sheets 链接的单元格写入使用独立命令面，不使用 `ixf docs update`。当前版本只支持 dry-run 规划；`ixf sheets update --apply` 尚不可用，直到真实写入 API 契约完成捕获和 fixture 覆盖。
...
- 简单表格、任务列表、代码块语言、富文本链接、图片块下载、直接 sheets 读取、嵌入 sheet 展开、sheets 更新 dry-run 规划和安全资源清理。
...
- `ixf sheets update --apply` 当前明确不可用；不要声称已支持远程 sheet 单元格写入。

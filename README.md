...
> 请帮我安装 https://github.com/serialq7ic4/ixf-toolbox。使用 GitHub Release Go 二进制安装本地 `ixf`（macOS Apple Silicon 用 `ixf_2.5.0_darwin_arm64`，macOS Intel 用 `ixf_2.5.0_darwin_amd64`，Windows 用 `ixf_2.5.0_windows_amd64.exe`），然后运行 `ixf setup skills --runtimes codex --json` 注册 skill，最后用 `ixf --version` 和 `ixf doctor --json` 验证。
...
curl -L -o ~/.local/bin/ixf \
  https://github.com/serialq7ic4/ixf-toolbox/releases/download/v2.5.0/ixf_2.5.0_darwin_arm64
...
macOS Intel 将文件名换成 `ixf_2.5.0_darwin_amd64`。

### Windows PowerShell

```powershell
New-Item -ItemType Directory -Force $HOME\bin | Out-Null
Invoke-WebRequest -Uri https://github.com/serialq7ic4/ixf-toolbox/releases/download/v2.5.0/ixf_2.5.0_windows_amd64.exe -OutFile $HOME\bin\ixf.exe
...
```bash
python -m pip install "ixf-toolbox[crypto] @ https://github.com/serialq7ic4/ixf-toolbox/releases/download/v2.5.0/ixf_toolbox-2.5.0-py3-none-any.whl"
ixf setup skills --runtimes auto --json
```

Windows 使用 Python legacy/reference 时仍可安装 `[windows]` extra。
...
v2.4 起 Go 二进制拥有已文档化的 CLI runtime：文档读取/发布、OKR 读取/写入、cookie export、doctor、skill setup 和 update flow。Python wheel 保留为 legacy/reference，用于回退、对照测试和 Python 包 API 调用；当前 [`docs/python-removal-readiness.md`](docs/python-removal-readiness.md) 的结论是暂不删除 Python。

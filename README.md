...
> 请帮我安装 https://github.com/serialq7ic4/ixf-toolbox。使用 GitHub Release Go 二进制安装本地 `ixf`（macOS Apple Silicon 用 `ixf_2.17.0_darwin_arm64`，macOS Intel 用 `ixf_2.17.0_darwin_amd64`，Windows 用 `ixf_2.17.0_windows_amd64.exe`），然后运行 `ixf setup skills --runtimes codex --json` 注册 skill，最后用 `ixf --version` 和 `ixf doctor --json` 验证。
...
curl -L -o ~/.local/bin/ixf \
  https://github.com/serialq7ic4/ixf-toolbox/releases/download/v2.17.0/ixf_2.17.0_darwin_arm64
...
macOS Intel 将文件名换成 `ixf_2.17.0_darwin_amd64`。

### Windows PowerShell

```powershell
New-Item -ItemType Directory -Force $HOME\bin | Out-Null
Invoke-WebRequest -Uri https://github.com/serialq7ic4/ixf-toolbox/releases/download/v2.17.0/ixf_2.17.0_windows_amd64.exe -OutFile $HOME\bin\ixf.exe

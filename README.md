...
> 请帮我安装 https://github.com/serialq7ic4/ixf-toolbox。使用 GitHub Release Go 二进制安装本地 `ixf`（macOS Apple Silicon 用 `ixf_3.6.0_darwin_arm64`，macOS Intel 用 `ixf_3.6.0_darwin_amd64`，Windows 用 `ixf_3.6.0_windows_amd64.exe`），然后运行 `ixf setup skills --runtimes codex --json` 注册 skill，最后用 `ixf --version` 和 `ixf doctor --json` 验证。
...
curl -L -o ~/.local/bin/ixf \
  https://github.com/serialq7ic4/ixf-toolbox/releases/download/v3.6.0/ixf_3.6.0_darwin_arm64
...
macOS Intel 将文件名换成 `ixf_3.6.0_darwin_amd64`。
...
- Messenger 自动化就绪诊断、profile 发现、profile 安全克隆、dry-run 打开规划、显式 --apply 打开并验证目标会话、只读读取最近/未读会话，以及确认后的消息发送和 fresh-session 复核。
...
- Messenger 当前支持诊断、dry-run 打开规划、显式 --apply 目标验证、只读会话读取，以及确认后的消息发送；发送成功必须通过 fresh-session 复核。

...
v1.x 仍以 Python 版作为默认安装方式和 reference runtime。Go CLI 正按阶段迁移：已覆盖 `--version`、`doctor`、`setup skills`、本地 `docs read/outline/chunk/inspect/cleanup`、授权远程 docx 文本和图片读取、基础 docx block 转 Markdown、`update check/self`、校验 checksum 后的 Go 二进制自升级和 `update skills`；cookie export、wiki/mindnote/sheet 完整远程 parity、docs publish、OKR read/write 仍以 Python 版为准，直到完成 golden parity 后再切默认安装方式。

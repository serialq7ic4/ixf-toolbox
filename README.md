...
v1.x 仍以 Python 版作为默认安装方式和 reference runtime。Go CLI 正按阶段迁移：已覆盖 `--version`、`doctor`、`setup skills`、本地 `docs read/outline/chunk/inspect/cleanup`、授权远程 docx 文本/图片/嵌入 sheet 读取、可解析到 docx token 的 wiki 读取、基础 docx block 转 Markdown、`update check/self`、校验 checksum 后的 Go 二进制自升级和 `update skills`；cookie export、wiki 变体/mindnote 完整远程 parity、docs publish、OKR read/write 仍以 Python 版为准，直到完成 golden parity 后再切默认安装方式。
...
- 可解析到受支持文档类型的 `wiki` 链接读取。
- 通过受支持文档 payload 暴露出来的 mindnote 标记和嵌入 sheet TSV 展开。

...
推荐让当前正在使用的 agent 直接完成安装。默认安装方式是下载 GitHub Release Go 二进制，再把五个 skill 注册到 Codex 或 Claude Code；不要求本机具备 Python 环境。
...
安装 skill 后，可以直接让 agent 处理授权链接或本地文件。你不需要点名具体 skill；直接按日常方式描述目标即可。using-ixf-toolbox 会在后台识别链接类型、读写意图和安全边界，再分配给对应的文档或 OKR skill。
...
v2.4 起 Go 二进制拥有已文档化的 CLI runtime：文档读取/发布、OKR 读取/写入、cookie export、doctor、skill setup 和 update flow。v2.6 起 GitHub Release 只发布 Go 二进制和 checksum；v3.0 起 Python runtime/package 实现已删除；v3.1 起测试和发布流程也不再依赖 Python。

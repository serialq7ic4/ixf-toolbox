...
v1.x 仍以 Python 版作为默认安装方式和 reference runtime。Go CLI 正按阶段迁移：已覆盖 `--version`、`doctor`、`setup skills`、本地 `docs read/outline/chunk/inspect/cleanup`、授权远程 docx 文本/图片/嵌入 sheet 读取、可解析到 docx token 的 wiki 读取、wiki bitable TSV 读取、直接 mindnote 树形读取、基础 docx block 转 Markdown、docs publish dry-run/apply、OKR read、OKR write dry-run、`update check/self`、校验 checksum 后的 Go 二进制自升级和 `update skills`；cookie export、剩余 wiki 边缘变体远程 parity、OKR write apply 仍以 Python 版为准，直到完成 golden parity 后再切默认安装方式。

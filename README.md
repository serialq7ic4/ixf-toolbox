...
推荐让当前正在使用的 agent 直接完成安装。安装过程会先安装 `ixf-toolbox` Python 包，再把五个 skill 注册到 Codex 或 Claude Code。

### 安装到 Codex

如果你正在使用 Codex，可以直接对 Codex 说：

> 请帮我安装 https://github.com/serialq7ic4/ixf-toolbox。使用 GitHub Release wheel 安装本地执行引擎（macOS 用 `[crypto]`，Windows 用 `[windows]`），然后运行 `ixf setup skills --runtimes codex --json` 注册 skill，最后用 `ixf --version` 和 `ixf doctor --json` 验证。

手动命令：

```bash
python -m pip install "ixf-toolbox[crypto] @ https://github.com/serialq7ic4/ixf-toolbox/releases/download/v1.7.0/ixf_toolbox-1.7.0-py3-none-any.whl"
...
```bash
python -m pip install "ixf-toolbox[windows] @ https://github.com/serialq7ic4/ixf-toolbox/releases/download/v1.7.0/ixf_toolbox-1.7.0-py3-none-any.whl"
```
...
v1.x 仍以 Python 版作为默认安装方式和 reference runtime。Go CLI 正按阶段迁移：已覆盖 `--version`、`doctor`、`setup skills`、本地 `docs read/outline/chunk/inspect/cleanup`、授权远程 docx 文本/图片/嵌入 sheet 读取、可解析到 docx token 的 wiki 读取、wiki bitable TSV 读取、直接 mindnote 树形读取、基础 docx block 转 Markdown、docs publish dry-run/apply、OKR read、OKR write dry-run、OKR `write --apply --objective-index`、cookie export、`update check/self`、校验 checksum 后的 Go 二进制自升级和 `update skills`；剩余 wiki 边缘变体远程 parity 和更宽泛的 OKR 变更流仍以 Python 版为准，直到完成 golden parity 后再切默认安装方式。
...
`--objective-index` 用于只修改指定 Objective，并验证其他 Objective 未被改变。`--prune` 会删除输入中没有保留的内容，仅在明确需要时使用。
...
- 授权 OKR 页面读取、指定 Objective 写入、KR 创建/修改和发布。
...
smoke 脚本会要求 `dist/` 中恰好存在一个 wheel，并在临时虚拟环境和临时 HOME 中验证 wheel 的包版本、CLI 入口和内置 Codex skill，不会使用全局已安装的 `ixf`。

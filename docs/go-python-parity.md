...
| `bitable record create` | Owned | `ixf bitable record create --dry-run` plans new records, including local file paths for attachment fields; `ixf bitable record create --apply` fails closed until the record-create API contract is captured. |

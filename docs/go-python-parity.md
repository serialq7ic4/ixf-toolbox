...
| `bitable record create` | Owned | `ixf bitable record create --dry-run` plans new records, including local file paths for attachment fields; `ixf bitable record create --apply` creates confirmed records through API-only RCE writes, uploads attachment files, and verifies by clientvars readback. |
| `bitable attach` | Owned | `ixf bitable attach --dry-run` plans attachment uploads into existing bitable attachment fields; `ixf bitable attach --apply` fails closed until the existing-record update API contract is captured. |

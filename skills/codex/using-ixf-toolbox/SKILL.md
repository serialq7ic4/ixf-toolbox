...
- For bitable record or attachment/image upload requests, use `ixf bitable inspect --url <url> --json`, `ixf bitable record create --dry-run --json`, or `ixf bitable attach --dry-run --json`; do not route those requests through docs or sheets commands. `ixf bitable record create --apply` and `ixf bitable attach --apply` are unavailable until the bitable write API contracts are captured.
...
9. For bitable record or attachment requests, run the relevant `ixf bitable ... --dry-run`, show the plan, and stop; do not claim remote writes are supported yet.

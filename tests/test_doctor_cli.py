...
def test_doctor_json_outputs_toolbox_diagnostics(monkeypatch, capsys):
    def fake_collect_diagnostics(*, home, env, cookies_path):
        ...
        return {"ok": True, "version": "1.1.0", "capabilities": {}, "skills": {}, "cookies": {}}
...
def test_doctor_text_outputs_toolbox_diagnostics(monkeypatch, capsys):
    monkeypatch.setattr(
        ...
        lambda *, home, env, cookies_path: {"ok": False, "version": "1.1.0"},
    )
    monkeypatch.setattr(
    ...
    assert capsys.readouterr().out == "ixf-toolbox 1.1.0\noverall fail\n"

...
def test_doctor_json_outputs_toolbox_diagnostics(monkeypatch, capsys):
    def fake_collect_diagnostics(*, home, env, cookies_path):
        ...
        return {"ok": True, "version": "2.5.1", "capabilities": {}, "skills": {}, "cookies": {}}
...
def test_doctor_text_outputs_toolbox_diagnostics(monkeypatch, capsys):
    monkeypatch.setattr(
        ...
        lambda *, home, env, cookies_path: {"ok": False, "version": "2.5.1"},
    )
    monkeypatch.setattr(
    ...
    assert capsys.readouterr().out == "ixf-toolbox 2.5.1\noverall fail\n"

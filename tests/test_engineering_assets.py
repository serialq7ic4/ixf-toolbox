...
def test_release_notes_script_extracts_non_empty_changelog_section():
    ...
    assert "remote docs reader Python runtime test" in result.stdout
...
def test_v2_docs_make_go_binary_the_default_install_path():
    ...
    assert "ixf_2.15.0_darwin_arm64" in zh
    ...
    assert "ixf_2.15.0_darwin_arm64" in en
...
def test_python_runtime_import_allowlist_shrinks_cli_and_update_contracts():
    ...
    for removed in [
        ...
        "tests/test_core_cookies_windows.py",
        "tests/test_core_docs.py",
        "tests/test_core_docs_assets.py",
        "tests/test_core_docs_converter.py",

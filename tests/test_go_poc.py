...
def test_go_ixf_version_matches_python_release(tmp_path):
    ...
    assert result.stdout.strip() == "ixf 2.3.0"
...
def test_go_ixf_update_self_json_defaults_to_dry_run_with_fixture(tmp_path):
    ...
    release.write_text(
        json.dumps(
            {
                "tag_name": "v2.4.0",
                "html_url": "https://github.example/releases/v2.4.0",
            }
        ),
        encoding="utf-8",
    )
    ...
    assert payload["ok"] is True
    assert payload["currentVersion"] == "2.3.0"
    assert payload["latestVersion"] == "2.4.0"
    assert payload["latestTag"] == "v2.4.0"
